...
If either can move because something unrelated changed elsewhere in the document,
two things break: links from outside rot,
and the committed Markdown fills with diff noise that a human has to read past.
...
Every output refers to them by this name, and the download writes them under it,
so it lives here rather than being spelled out
in each of the four places that has to agree with the other three.
Serving them from somewhere else is what an emitter's `image_base` is for,
and is a decision about hosting rather than about a build.
"""

# What a filename may keep:
# a dot is as much a part of a name as a letter is,
# and `-` and `_` are the two characters people join names with.
_NON_FILENAME = re.compile(r"[^\w.-]")
# The extension a source line writes names the format rather than the picture,
# and the one that publishes is whatever the download turns out to fetch,
# so it is dropped and the real one appended in its place.
_ASSET_EXTENSION = re.compile(r"\.(?:jpe?g|png|gif|webp|svg|pdf|tiff?|heic)$", re.IGNORECASE)


# GitHub's rule for a heading anchor,
# as implemented by `pymdown-extensions` rather than by this file.
# The two rules agreeing today is not a reason to write a second copy of one:
# they agree until a heading has an accent in it,
# and then whichever copy is wrong is wrong in published URLs.
# This is the same implementation MkDocs Material publishes GitHub-style slugs with.
#
# The dedicated port of GitHub's slugger on PyPI, `github-slugger`,
# cannot be imported at all on a current Python:
# it carries the JavaScript regex as lone surrogates,
# which is a `UnicodeEncodeError` on import.
...
def slugify(text: str) -> str:
    ...
    This project publishes the same report as HTML and as Markdown,
    and a link to a section has to mean the same thing in both,
    so the rule is not this project's to choose:
    lowercase, punctuation dropped, `-` and `_` kept, one hyphen per space,
    and letters that are not ASCII kept as they are,
    which is what GitHub renders an anchor as.

    Not the rule a filename gets.
    A heading is a sentence, and every separator in its anchor is one this invented;
...
def _ascii_name(name: str) -> str:
    ...
    Every space and every character that cannot appear becomes one underscore,
    and nothing else moves:
    runs are not collapsed, case is not folded, and a dot is left where it was.
    `SAS West - Tunnel Profile - pg 18.screenshot`
    publishes under a name its author would recognize,
    which is the whole reason for reading the source line at all.
    """
...
class AnchorAllocator:
    ...
    Allocation is two-pass on purpose.
    ...
    both of their anchors move.

    So the constructor is given every heading text up front,
    works out which base slugs more than one distinct heading would claim,
    and suffixes *all* claimants of those.
    A heading's anchor is then a pure function of its own text
    plus the set of headings it collides with,
    ...
    `reserved` holds ids the emitters generate themselves,
    such as the `footnotes` section.
    A heading actually titled "Footnotes" would otherwise collide with it,
    producing exactly the duplicate id that is one of the three defects on the live page.
    """
...
def content_anchor(prefix: str, text: str) -> str:
    ...
    Paragraphs, tables, and lists carry no identifier in the Docs API
    and no title to slugify, but they still want to be linkable:
    a report this long is quoted a paragraph at a time.

    Hashing what the block says rather than counting where it sits
    is the same trade the rest of this module makes,
    and here it is the difference between two failures.
    Numbering paragraphs means inserting one
...
def image_filename(object_id: str, extension: str = "", crop_key: str = "", name: str = "") -> str:
    ...
    A `Source:` line is the document stating which file this is,
    which is what makes it usable as a name where a caption is not:
    a caption describes the picture and gets rewritten in copy-editing,
    and a position changes whenever anything is inserted above it.
    Editing a source line does move a published URL,
    and that is the cost of the names being readable at all.

    Most images are given no source line,
    and those keep the name they always had: their Docs object id, hashed.
    The id is stable across edits,
    so inserting an image cannot rename the ones around it.
    ...
    The extension is filled in
    once the image is downloaded and its real content type is known.
    """
...
def image_filenames(claims: Iterable[tuple[str, str, str]]) -> dict[str, str]:
    ...
    which is empty for the images it named nothing.
    ...
    So both of them keep the name and carry the hash as well,
    which is the same way a crop is told from its original.
    ...
    for object_id, crop_key, name in claims:
        ...
        # and asking for it here is what says why:
        # the two images say they are the same file, and only the hash can disagree.
        names[object_id] = (
