...
#show: report.with(
  ...
  warnings: (
    ...
    [style: #raw("8 minutes") is #raw("8 min"), which is how a report writes a measurement: #raw("...l city. SkyTrain runs every 6-8 minutes off-peak on each of two branc...")],
    [style: #raw("6 minutes") is #raw("6 min"), which is how a report writes a measurement: #raw("...Copenhagen Metro runs every 4-6 minutes off-peak on each of two branc...")],
    [style: #raw("3-minute") is #raw("3-min"), which is how a report writes a measurement: #raw("...f city center, offering about 3-minute frequencies on a lengthy trun...")],
    [style: #raw("6 minutes") is #raw("6 min"), which is how a report writes a measurement: #raw("...ss VAL technology run every 3-6 minutes off-peak in Lille, Rennes, To...")],
    [style: #raw("15 minutes") is #raw("15 min"), which is how a report writes a measurement: #raw("...Marseille Metro runs every 10-15 minutes.")],
    [style: #raw("3-minute") is #raw("3 min"), with no hyphen between a number and its unit: #raw("...f city center, offering about 3-minute frequencies on a lengthy trun...")],
    ...
    [style: #raw("5-minute") is #raw("5-min"), which is how a report writes a measurement: #raw("...time and the currently stated 5-minute peak headways. If peak headwa...")],
    [style: #raw("2 minutes") is #raw("2 min"), which is how a report writes a measurement: #raw("... peak headways are reduced to 2 minutes instead, with off-peak headwa...")],
    [style: #raw("3 minutes") is #raw("3 min"), which is how a report writes a measurement: #raw("...ff-peak headways ranging from 3 minutes midday to 5 minutes in the ev...")],
    [style: #raw("5 minutes") is #raw("5 min"), which is how a report writes a measurement: #raw("...ging from 3 minutes midday to 5 minutes in the evening, ridership ris...")],
    [style: #raw("5-minute") is #raw("5 min"), with no hyphen between a number and its unit: #raw("...time and the currently stated 5-minute peak headways. If peak headwa...")],
    [style: #raw("66 seconds") is #raw("66 sec"), which is how a report writes a measurement: #raw("...rains, but it runs them every 66 seconds, and can run up to 60 seconds...")],
    [style: #raw("60 seconds") is #raw("60 sec"), which is how a report writes a measurement: #raw("...66 seconds, and can run up to 60 seconds. While a light metro can be a...")],
    [style: #raw("1-min") is #raw("1 min"), with no hyphen between a number and its unit: #raw("...frequencies, like the extreme 1-min headways of Lille to the more...")],
    [style: #raw("2-min") is #raw("2 min"), with no hyphen between a number and its unit: #raw("... Lille to the more normal 1.5-2-min minimum technical headways of...")],
    [style: #raw("2 minutes") is #raw("2 min"), which is how a report writes a measurement: #raw("...sist) at very high frequency (2 minutes apart or less), trading per-t...")],
    [style: #raw("600-foot") is #raw("600-ft"), which is how a report writes a measurement: #raw("...equire the high capacity of a 600-foot subway train. Since most IBX ...")],
    [style: #raw("2 minutes") is #raw("2 min"), which is how a report writes a measurement: #raw("...trains as frequently as every 2 minutes or less thus benefits the pla...")],
    [style: #raw("600-foot") is #raw("600 ft"), with no hyphen between a number and its unit: #raw("...equire the high capacity of a 600-foot subway train. Since most IBX ...")],
    [style: #raw("180 meters") is #raw("180 m"), which is how a report writes a measurement: #raw("... on the New York City Subway (180 meters) or longer exclusively use hi...")],
    [style: a sentence should end with 1 space, not 2: #raw("...t costs are given separately).··This is a Buy America cost, wh...")],
    [style: #raw("32-min") is #raw("32 min"), with no hyphen between a number and its unit: #raw("With a 32-min runtime, 2-min turns, and a 9...")],
    [style: #raw("2-min") is #raw("2 min"), with no hyphen between a number and its unit: #raw("With a 32-min runtime, 2-min turns, and a 90% utilization ...")],
    [style: #raw("2-min") is #raw("2 min"), with no hyphen between a number and its unit: #raw("... fleet cost. But this is with 2-min peak headways, not 5-min peak...")],
    [style: #raw("5-min") is #raw("5 min"), with no hyphen between a number and its unit: #raw("...with 2-min peak headways, not 5-min peak headways. And with a 92%...")],
    [style: #raw("72,000 lbs") is #raw("72,000 lb"), which is how a report writes a measurement: #raw("weight: 72,000 lbs per 65 ft AW0 car")],
    [style: #raw("72,000 lbs") is #raw("72,000 lb"), which is how a report writes a measurement: #raw("AW0 weight/length: 72,000 lbs / 65 ft = 1648 kg/m")],
    [style: #raw("175 lbs") is #raw("175 lb"), which is how a report writes a measurement: #raw("AW0 weight/length: (232 t - (175 lbs * 780)) / (19.05 m * 4) = 223...")],
  ),
)
...
While a light metro can be as small and limited as an airport’s people mover system, it can also be as large as the #link("https://rail.kychung.com/en/2d-en/guangzhou-metro-line-6-csr-sifang-emu/")[narrow 6B trains]#super[#link(<src2>)[\[2\]]] serving Guangzhou Metro Line 3, which carried #link("http://sn.people.com.cn/n2/2020/0102/c378296-33682040.html")[2.76 million riders a day]#super[#link(<src3>)[\[3\]]] in 2019 and is perhaps the busiest metro line in the world.#footnote[As far as we are aware, the only rapid transit lines that carry more riders per day are Tokyo’s Yamanote Line (~5 million/day) and Mumbai Suburban Railway’s Western Line (~3 million/day).
However, both are technically mainline rail lines, and Mumbai’s Western Line is quad-tracked for much of its length.]
To be fair, Guangzhou Metro Line 3 is ridiculously over-capacity (it makes the pre-Second Avenue Subway phase 1 opening Lexington Av line look roomy, and has needed multiple relief lines and faces crowding limitations at many stations) and should have been heavy metro for sure, but it is a testament to the potential of light metro that it can carry the busiest metro line in the world.

That said, while ridership keeps climbing, the IBX will never be as busy as Guangzhou Metro Line 3.
With the current 160k daily ridership projection, however, it will already be #link("https://www.mta.info/press-release/icymi-governor-hochul-announces-interborough-express-advancing-planning-active-phase#:~:text=IBX%E2%80%99s%20projected%20annual%20ridership%20is%20higher%20than%20the%20current%20ridership%20of%20any%20other%20light%20rail%20system%20in%20the%20country%20at%2048%20million%20riders%20%E2%80%94%20the%20next%20largest%20is%20Los%20Angeles%20at%2046%20million%20riders%20per%20year.")[the busiest light rail system (not line, whole system) in the US]#super[#link(<src4>)[\[4\]]].
...
#underline[#strong[\[below is more just notes or old stuff\]]]
...
Additionally, ALM trains require smaller station footprints, an essential benefit given the outsized role #link("https://transitcosts.com/city/newyork-case-study/")[station size]#super[#link(<src5>)[\[5\]]] plays in the MTA’s high construction costs.
...
There are international standards for different #link("https://en.wikipedia.org/wiki/Automatic_train_operation#Grades_of_automation")[#emph[grades of automation]]#super[#link(<src6>)[\[6\]]], signifying exactly how automated a transit line’s operations is.
The most automated of these grades is the 4th grade (#emph[GoA4)].
...
If it follows through, it would, happily, break a trend of insisting carmakers comply with exacting standards, which started back in 1977 in the wake of the #link("https://timesmachine.nytimes.com/timesmachine/1980/09/27/111295852.html?pageNumber=16")[R46 truck-cracking debacle]#super[#link(<src7>)[\[7\]]].
...
cost: #link("https://www.govtech.com/fs/honolulus-first-two-rail-cars-for-new-transit-system-arrive-on-oahu.html#:~:text=cars%2C%20valued%20at-,%242.2%20million%20each,-and%20designed%20to")[\$2.2 million/car]#super[#link(<src9>)[\[9\]]] (#link("http://hartdocs.honolulu.gov/docushare/dsweb/Get/Document-14729/Agreement.pdf")[contract in 2011]#super[#link(<src10>)[\[10\]]], #link("https://www.govtech.com/fs/honolulus-first-two-rail-cars-for-new-transit-system-arrive-on-oahu.html")[first delivered on March 25, 2016]#super[#link(<src11>)[\[11\]]], #link("https://www.masstransitmag.com/rail/vehicles/press-release/55021467/honolulu-authority-for-rapid-transportation-hart-honolulu-accepts-delivery-of-20th-train")[last delivered on April 24, 2024]#super[#link(<src12>)[\[12\]]])
...
weight: #link("http://www.staradvertiser.com/news/20110322_Have_a_seat.html#:~:text=to%20weigh%20about-,72%2C000%20pounds,-.")[72,000 lbs per 65 ft AW0 car]#super[#link(<src13>)[\[13\]]]
...
weight: #link("https://rem.info/en/rolling-stock#:~:text=Maximum%20weight%3A%20232%20tonnes")[232 t per 76.2 m AW3 4-car trainset]#super[#link(<src14>)[\[14\]]]
