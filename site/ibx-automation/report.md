...
- no TITLE-styled paragraph and no `Title:` header field, so the document name 'IBX Automation' is being used as the headline; style the headline as Title in the doc to fix this
...
- no `URL:` in the header, so this is published at /ibx-automation, which is a guess; add the line to the document
...
- the `Header` section has no `URL:` line
...
Peak hourly ridership on the 4 and 5 trains from the Upper East Side to Midtown was 30,000 before Second Avenue Subway opened \[source: Hub Bound 2015-16 data, and SAS EIS\], whereas ALM is typically designed for 15,000-25,000.[^1]
...
[^4]: As far as we are aware, the only rapid transit lines that carry more riders per day are Tokyo’s Yamanote Line (~5 million/day) and Mumbai Suburban Railway’s Western Line (~3 million/day).
...
- Alon Levy (al2495@gmail.com)
- Franklin Tang (zt2339@nyu.edu) Madison Feinberg (Madison Feinberg) Robert Hale (Robert Hale)
- Khyber Sen (Khyber Sen)
